// Phase 2 Implementation: Model Layer
...
void TestResourceCallbacks() {
    ...
    ResourceEvent callback_event;
    std::string callback_details;
    
    model_info.RegisterResourceCallback([&](const std::string& model_id, ResourceEvent event, const std::string& details) {
        callback_called = true;
        callback_model_id = model_id;
        callback_event = event;
        callback_details = details;
        std::cout << "Resource callback: model=" << model_id << ", event=" << static_cast<int>(event) << ", details=" << details << std::endl;
    });
...
int TestPhase2Model() {
    ...
    try {
        Phase2Tests::TestModelInfoCreation();
        std::cout << std::endl;
        
        Phase2Tests::TestTokenCacheIntegration();
        std::cout << std::endl;
        
        Phase2Tests::TestSettingsIntegration();
        std::cout << std::endl;
        
        Phase2Tests::TestResourceCallbacks();
        std::cout << std::endl;
        
        Phase2Tests::TestModelLifecycle();
        std::cout << std::endl;
        
        Phase2Tests::TestMemoryManagement();
        std::cout << std::endl;
        
        std::cout << "Model layer components are working correctly!" << std::endl;
        
    } catch (const std::exception& e) {
        std::cerr << "Phase 2 test failed with exception: " << e.what() << std::endl;
        return 1;
    }
