...
class TokenCache {
public:
    // Direct access to cache statistics (Directive #7: favor direct access over accessors)
...
public:
    explicit TokenCache(const size_t max_size = DEFAULT_CACHE_SIZE) : 
        cache_hits(0), cache_requests(0), max_cache_size(max_size) {}
    
    // Resize cache with automatic trimming if needed (replaces set_max_size accessor)
    ...
    void clear() const {
      ...
      // Direct access to cache statistics (Directive #7: favor direct access over accessors)
    // Access via: cache_hits, cache_requests, token_cache.size(), max_cache_size
    // Hit ratio: (cache_requests > 0) ? static_cast<float>(cache_hits) / static_cast<float>(cache_requests) : 0.0f
    // Fill ratio: static_cast<float>(token_cache.size()) / static_cast<float>(max_cache_size)
